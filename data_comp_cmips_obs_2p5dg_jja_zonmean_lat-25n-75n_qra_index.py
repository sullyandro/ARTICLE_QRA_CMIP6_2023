...
for exp in ['cmip6']:
    ...
    for subexp in ['ssp370']:
        ...
        dir_ou = '../data/{0}/{1}/{2}_2p5dg_jja_zonmean_lat-25n-75n_QRAindex'.format(exp, subexp, var)

        if not os.path.exists(dir_ou): os.makedirs(dir_ou)

        # Temperature anomaly ENSEMBLE-MEAN

        fnames = '{0}/{3}_Amon_ENSEMBLE-MEAN_{2}_*_{4}_2p5dg_jja_zonmean_lat-25n-75n_anom.nc'.format(dir_in, exp, subexp, var, period)
        files = glob(fnames)
        ...
        if exp in ['cmip5'] and subexp in ['historical']:
            clim_year_1, clim_year_2 = 1880, 2005

        if exp in ['cmip5'] and subexp in ['rcp85']:
            clim_year_1, clim_year_2 = 2006, 2100

        if exp in ['cmip6'] and subexp in ['historical']:
            clim_year_1, clim_year_2 = 1880, 2005
            
        if exp in ['cmip6'] and subexp in ['ssp585', 'ssp370']:
            clim_year_1, clim_year_2 = 2015, 2100
        ...
        # print('OFFSET -->', OFFSET)
        # exit()
        offset_dic[exp+'_'+subexp] = OFFSET
            
        # plt.plot(range(11), OFFSET, label=exp+'_'+subexp)
        
        # Temperature anomaly Models

        fnames = '{0}/{3}_Amon_*_{2}_*_{4}_2p5dg_jja_zonmean_lat-25n-75n_anom.nc'.format(dir_in, exp, subexp, var, period)
        print('fnames -->', fnames)
        files = glob(fnames)
        files.sort()
        ...
        for a in files:
            ...
            if subexp in ['rcp85', 'ssp585', 'ssp370']:
                tas_mod = dat[:] #+ offset_dic[exp+'_historical'][:]
            else: 
                tas_mod = dat[:] + offset_dic[exp+'_'+subexp][:]
            ...
            fou = '{0}/{3}_Amon_{4}_{2}_{5}_{6}_2p5dg_jja_zonmean_lat-25n-75n_QRAindex.nc'.format(dir_ou, exp, subexp, var, model_name, run, period)
        ...
        # y   = qra_index_mod_mean[:,0,0]
        # yrs = np.arange(iyr,iyr+len(y))

        # plt.plot(yrs, y)
        # for a in names:
            # plt.plot(yrs, qra_all[a][:,0,0])
            
        # plt.xlim(yrs[0], yrs[-1])
        # plt.xticks(range(yrs[0], yrs[-1], 5), range(yrs[0], yrs[-1], 5))
        # plt.show()
...
for i in offset_dic:
    print()
    name = i+'         '
    print(name[:20], offset_dic[i], np.mean(offset_dic[i]))
print()
