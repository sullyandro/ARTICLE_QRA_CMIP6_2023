...
#
# Description:
# Script to get CMIP data from PIK database, interpolate to 2.5dg, compute JJA mean, and zonmean.
# 
################################################################################
...
case = 1	# Choose here which case should run
...
for a in bigdic:
	...
	if var in ['tas', 'ua']:
		...
		for i in fnames:
			...
			if var == 'tas':
				cmd = 'cdo -s -O zonmean -selindexbox,1,144,37,73 -seasmean -remapbil,griddes-2.5deg {3} -selmonth,6,7,8                 -selname,{2} {0} {1}'.format(full_file_in, full_file_ou, var, cmd_mon)
			if var == 'ua':
				cmd = 'cdo -s -O zonmean -selindexbox,1,144,37,73 -seasmean -remapbil,griddes-2.5deg {3} -selmonth,6,7,8 -sellevel,30000 -selname,{2} {0} {1}'.format(full_file_in, full_file_ou, var, cmd_mon)
