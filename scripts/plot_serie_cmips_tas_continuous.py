...
# GISSTEMP
fin = '../data/Mann_etal_2018/GISTEMP_latband_2017_1880-2016_jja_lat-25n-75n.nc'
print('input -->', fin)
ds  = xr.open_dataset(fin, decode_times=False)
gis = np.squeeze(ds['tas'].data)
gis = np.nanmean( np.squeeze(ds['tas'].data), axis=1) + 15.7
yrs_gis = range(1880, 2016+1)

# HadCRUT
fin = '../data/Mann_etal_2018/HadCRUT_latband_2017_1850-2016_jja_lat-25n-75n.nc'
print('input -->', fin)
ds  = xr.open_dataset(fin, decode_times=False)
had = np.nanmean( np.squeeze(ds['tas'].data), axis=1) + 15.7
yrs_had = range(1850, 2016+1)

# Cowtan & Way
fin = '../data/Mann_etal_2018/HadCRUT_krig_latband_2017_1850-2016_jja_lat-25n-75n.nc'
print('input -->', fin)
ds  = xr.open_dataset(fin, decode_times=False)
cow = np.nanmean( np.squeeze(ds['tas'].data), axis=1) + 15.7
...
for exp in ['cmip5', 'cmip6']:
	...
	for subexp in subex:
		...
		for i,a in enumerate(files):
			...
			if 'ENSEMBLE' in a: continue
			
			# historical
			
			ah = a.replace('_anom', '')
			ah = ah.replace('_continuous', '')
			ah = ah.replace('185001-210012', exp_dates[exp]['historical'])
			ah = ah.replace(subexp, 'historical')
			
			print(ah)
			
			ds  = xr.open_dataset(ah, decode_times=False)
			dat_h = np.nanmean( np.squeeze(ds['tas'].data), axis=1)
			
			# Future
			
			af = a.replace('_anom', '')
			af = af.replace('_continuous', '')
			af = af.replace('185001-210012', exp_dates[exp][subexp.replace('historical-','')])
			af = af.replace(subexp, subexp.replace('historical-',''))
			
			print(af)
			
			ds  = xr.open_dataset(af, decode_times=False)
			dat_f = np.nanmean( np.squeeze(ds['tas'].data), axis=1)
			
			dat = np.concatenate((dat_h,dat_f))  - 273.15
...
data_ens_cmip5_hist                = np.nanmean(np.vstack(list(data_all_cmip5_hist.values())), axis=0) # data['cmip5']['historical']['ENSEMBLE-MEAN_r1i1p1_null']
data_tim_cmip5_hist                = exp_years['cmip5']['historical']

data_all_cmip5_rcp85               = data['cmip5']['historical-rcp85']
data_ens_cmip5_rcp85               = np.nanmean(np.vstack(list(data_all_cmip5_rcp85.values())), axis=0) # data['cmip5']['historical']['ENSEMBLE-MEAN_r1i1p1_null']
data_tim_cmip5_rcp85               = exp_years['cmip5']['rcp85']


# Plot [CMIP6 --> Historical and SSP585]  --> All models

data_all_cmip6_hist_ssp585         = data['cmip6']['historical-ssp585']
data_ens_cmip6_hist_ssp585         = np.nanmean(np.vstack(list(data_all_cmip6_hist_ssp585.values())), axis=0) # data['cmip6']['historical']['ENSEMBLE-MEAN_r1i1p1f1_gn']
data_tim_cmip6_hist_ssp585         = exp_years['cmip6']['historical']

data_all_cmip6_ssp585              = data['cmip6']['historical-ssp585']
data_ens_cmip6_ssp585              = np.nanmean(np.vstack(list(data_all_cmip6_ssp585.values())), axis=0) # data['cmip6']['historical']['ENSEMBLE-MEAN_r1i1p1f1_gn']
data_tim_cmip6_ssp585              = exp_years['cmip6']['ssp585']


# Plot [CMIP6 --> Historical and SSP370]  --> All models

data_all_cmip6_hist_ssp370         = data['cmip6']['historical-ssp585']
data_ens_cmip6_hist_ssp370         = np.nanmean(np.vstack(list(data_all_cmip6_hist_ssp370.values())), axis=0) # data['cmip6']['historical']['ENSEMBLE-MEAN_r1i1p1f1_gn']
data_tim_cmip6_hist_ssp370         = exp_years['cmip6']['historical']

data_all_cmip6_ssp370              = data['cmip6']['historical-ssp370']
data_ens_cmip6_ssp370              = np.nanmean(np.vstack(list(data_all_cmip6_ssp370.values())), axis=0) # data['cmip6']['historical']['ENSEMBLE-MEAN_r1i1p1f1_gn']
...
data_ens_cmip6_hist_ssp370_lowntcf = np.nanmean(np.vstack(list(data_all_cmip6_hist_ssp370_lowntcf.values())), axis=0) # data['cmip6']['historical']['ENSEMBLE-MEAN_r1i1p1f1_gn']
data_tim_cmip6_hist_ssp370_lowntcf = exp_years['cmip6']['historical']

data_all_cmip6_ssp370_lowntcf      = data['cmip6']['historical-ssp370-lowNTCF']
data_ens_cmip6_ssp370_lowntcf      = np.nanmean(np.vstack(list(data_all_cmip6_ssp370_lowntcf.values())), axis=0) # data['cmip6']['historical']['ENSEMBLE-MEAN_r1i1p1f1_gn']
...
figname = '../figures/plot_serie_cmip5_historical_rcp85_tas_1850-2100_continuous.png'

fig, ax = plt.subplots(figsize=(12,9))

plt.grid(color='grey', linestyle='--', linewidth=0.3, alpha=0.5)

plt.title('TAS - CMIP5 Historical and RCP85\nJJA mean - Zonal mean (25N-75N)')
plt.ylabel('Temperature [°C]')

plt.axvline(x=2005, ymin=0.1, ymax=0.95, color='gray', alpha=0.7, linestyle='--', lw=0.7)
plt.text(2004, 10.5, str(2005), color="gray", fontsize=8, rotation=90, alpha=0.7)
...
figname = '../figures/plot_serie_cmip6_historical_ssp585_tas_1850-2100_continuous.png'

fig, ax = plt.subplots(figsize=(12,9))

plt.grid(color='grey', linestyle='--', linewidth=0.3, alpha=0.5)

plt.title('TAS - CMIP6 Historical and SSP585\nJJA mean - Zonal mean (25N-75N)')
plt.ylabel('Temperature [°C]')

plt.axvline(x=2014, ymin=0.1, ymax=0.95, color='gray', alpha=0.7, linestyle='--', lw=0.7)
plt.text(2013, 10.5, str(2014), color="gray", fontsize=8, rotation=90, alpha=0.7)

for i,a in enumerate(data_all_cmip6_ssp585.keys()):
	if 'ENSEMBLE-MEAN' not in a:  ax.plot(data_tim_cmip6_hist_ssp585,   data_all_cmip6_hist_ssp585[a][lim_c6_hist], '-', color='gray', lw=1)
...
ax.plot(yrs_had, had, '-', color='green',  lw=1, label='HadCRUT',        alpha=0.9)
ax.plot(yrs_cow, cow, '-', color='orange', lw=1, label='COWTAN and WAY', alpha=0.9)
ax.plot(data_tim_cmip6_hist_ssp585, data_ens_cmip6_hist_ssp585[lim_c6_hist],   '-', color='black', lw=3, label='CMIP6 ENSEMBLE MEAN Historical')
ax.plot(data_tim_cmip6_ssp585,      data_ens_cmip6_ssp585     [lim_c6_ssp585], '-', color='black', lw=3, label='CMIP6 ENSEMBLE MEAN SSP585')

top_labels = 5

yrs = data_tim_cmip6_hist_ssp585.tolist() + data_tim_cmip6_ssp585.tolist()
ax.set_xlim(yrs[0], yrs[-1]+1)
ax.set_xticks(range(yrs[0], yrs[-1]+10, 10))
ax.xaxis.set_tick_params(labelsize=8)
ax.set_ylim(10, 26)
...
figname = '../figures/plot_serie_cmip6_historical_ssp370_tas_1850-2100_continuous.png'

fig, ax = plt.subplots(figsize=(12,9))

plt.grid(color='grey', linestyle='--', linewidth=0.3, alpha=0.5)

plt.title('TAS - CMIP6 Historical and SSP370\nJJA mean - Zonal mean (25N-75N)')
plt.ylabel('Temperature [°C]')
...
for i,a in enumerate(data_all_cmip6_ssp370.keys()):
	if 'ENSEMBLE-MEAN' not in a:  ax.plot(data_tim_cmip6_hist_ssp370, data_all_cmip6_hist_ssp370[a][lim_c6_hist], '-', color='gray', lw=1)
...
ax.plot(yrs_had, had, '-', color='green',  lw=1, label='HadCRUT',        alpha=0.9)
ax.plot(yrs_cow, cow, '-', color='orange', lw=1, label='COWTAN and WAY', alpha=0.9)
ax.plot(data_tim_cmip6_hist_ssp370, data_ens_cmip6_hist_ssp370[lim_c6_hist],   '-', color='black', lw=3, label='CMIP6 ENSEMBLE MEAN Historical')
ax.plot(data_tim_cmip6_ssp370,      data_ens_cmip6_ssp370     [lim_c6_ssp370], '-', color='black', lw=3, label='CMIP6 ENSEMBLE MEAN SSP370')

top_labels = 5

yrs = data_tim_cmip6_hist_ssp370.tolist() + data_tim_cmip6_ssp370.tolist()
ax.set_xlim(yrs[0], yrs[-1]+1)
ax.set_xticks(range(yrs[0], yrs[-1]+10, 10))
ax.xaxis.set_tick_params(labelsize=8)
ax.set_ylim(10, 26)
...
figname = '../figures/plot_serie_cmip6_historical_ssp370-lowNTCF_tas_1850-2100_continuous.png'

fig, ax = plt.subplots(figsize=(12,9))

plt.grid(color='grey', linestyle='--', linewidth=0.3, alpha=0.5)

plt.title('TAS - CMIP6 Historical and SSP370-lowNTCF\nJJA mean - Zonal mean (25N-75N)')
plt.ylabel('Temperature [°C]')
...
for i,a in enumerate(data_all_cmip6_ssp370_lowntcf.keys()):
	if 'ENSEMBLE-MEAN' not in a:  ax.plot(data_tim_cmip6_hist_ssp370_lowntcf, data_all_cmip6_hist_ssp370_lowntcf[a][lim_c6_hist],           '-', color='gray', lw=1)

for i,a in enumerate(data_all_cmip6_ssp370_lowntcf.keys()):
	if 'ENSEMBLE-MEAN' not in a:  ax.plot(data_tim_cmip6_ssp370_lowntcf,      data_all_cmip6_ssp370_lowntcf     [a][lim_c6_ssp370_lowntcf], '-', color='gray', lw=1, label='{1}_{2}_{0}'.format(*a.split('_')) )

ax.plot(yrs_gis, gis, '-', color='red',    lw=1, label='GISTEMP',        alpha=1)
ax.plot(yrs_had, had, '-', color='green',  lw=1, label='HadCRUT',        alpha=0.9)
ax.plot(yrs_cow, cow, '-', color='orange', lw=1, label='COWTAN and WAY', alpha=0.9)
ax.plot(data_tim_cmip6_hist_ssp370_lowntcf, data_ens_cmip6_hist_ssp370_lowntcf[lim_c6_hist],           '-', color='black', lw=3, label='CMIP6 ENSEMBLE MEAN Historical')
ax.plot(data_tim_cmip6_ssp370_lowntcf,      data_ens_cmip6_ssp370_lowntcf     [lim_c6_ssp370_lowntcf], '-', color='black', lw=3, label='CMIP6 ENSEMBLE MEAN SSP370-lowNTCF')

top_labels = 5

yrs = data_tim_cmip6_hist_ssp370_lowntcf.tolist() + data_tim_cmip6_ssp370_lowntcf.tolist()
ax.set_xlim(yrs[0], yrs[-1]+1)
ax.set_xticks(range(yrs[0], yrs[-1]+10, 10))
ax.xaxis.set_tick_params(labelsize=8)
ax.set_ylim(10, 26)
...
figname = '../figures/plot_serie_cmip5_cmip6_historical_rcp_ssp_tas_1850-2100_continuous.png'

fig, ax = plt.subplots(figsize=(12,8))

plt.grid(color='grey', linestyle='--', linewidth=0.3, alpha=0.5)

plt.title('TAS - CMIP5 and CMIP6\nJJA mean - Zonal mean (25N-75N)')
plt.ylabel('Temperature [°C]')

plt.axvline(x=2005, ymin=0.1, ymax=0.95, color='gray', alpha=0.7, linestyle='--', lw=0.7)
plt.axvline(x=2014, ymin=0.1, ymax=0.95, color='gray', alpha=0.7, linestyle='--', lw=0.7)
plt.text(2004, 14.3, str(2005), color="gray", fontsize=8, rotation=90, alpha=0.7)
plt.text(2013, 14.3, str(2014), color="gray", fontsize=8, rotation=90, alpha=0.7)
...
ax.plot(data_tim_cmip5_hist,        data_ens_cmip5_hist       [lim_c5_hist],   '-', color='dimgrey', lw=3, label='CMIP5 ENSEMBLE MEAN Historical')
ax.plot(data_tim_cmip5_rcp85,       data_ens_cmip5_rcp85      [lim_c5_rcp85],  '-', color='dimgrey', lw=3, label='CMIP5 ENSEMBLE MEAN RCP85')

yrs = data_tim_cmip6_hist_ssp585.tolist() + data_tim_cmip6_ssp370.tolist()
ax.set_xlim(yrs[0], yrs[-1]+1)
ax.set_xticks(range(yrs[0], yrs[-1]+10, 10))
ax.xaxis.set_tick_params(labelsize=8)
# ax.set_ylim(-1, 7)
...
figname = '../figures/plot_serie_cmip5_cmip6_historical_rcp_ssp_tas_1850-2100_with_SSP370-lowNTCF_continuous.png'
...
ax.plot(data_tim_cmip6_ssp585,              data_ens_cmip6_ssp585             [lim_c6_ssp585],         '-', color='blue',      lw=3, label='CMIP6 ENSEMBLE MEAN SSP585')
ax.plot(data_tim_cmip6_ssp370,              data_ens_cmip6_ssp370             [lim_c6_ssp370],         '-', color='sienna',    lw=3, label='CMIP6 ENSEMBLE MEAN SSP370')
ax.plot(data_tim_cmip6_ssp370_lowntcf,      data_ens_cmip6_ssp370_lowntcf     [lim_c6_ssp370_lowntcf], '-', color='darkgreen', lw=3, label='CMIP6 ENSEMBLE MEAN SSP370-lowNTCF')
ax.plot(data_tim_cmip6_hist_ssp370_lowntcf, data_ens_cmip6_hist_ssp370_lowntcf[lim_c6_hist],           '-', color='darkgreen', lw=3,)#label='CMIP6 ENSEMBLE MEAN Historical / SSP370-lowNTCF')
