...
for exp in ['cmip5', 'cmip6']:
	...
	for subexp in subex:
		...
		figname = '../figures/plot_ua_QRAindex_{0}_{1}_{2}.png'.format(exp, subexp, exp_dates[exp][subexp])

		fig, ax = plt.subplots(figsize=(10,7))

		plt.grid(color='grey', linestyle='--', linewidth=0.3, alpha=0.5)

		plt.title('UA from QRA Index regression {0} {1}'.format(exp.upper(), subexp.title()))
		...
		for i,a in enumerate(names):			
			...
			if model not in ['ENSEMBLE-MEAN', 'ERA-Interim']:
...
exp    = 'cmip5'
subexp = 'historical'
dir_in = '../data/{0}/{1}/{2}_2p5dg_jja_zonmean_lat-25n-75n_QRAindex'.format(exp, subexp, 'ua')
...
figname = '../figures/plot_ua_QRAindex_cmip5_cmip6_hist_proj.png'.format(exp, subexp)
