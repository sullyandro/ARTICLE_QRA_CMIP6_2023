...
for exp in ['cmip5', 'cmip6']:
    ...
    for subexp in subex:
        ...
        fnames = '{0}/{3}_Amon_*_{2}_*_{4}_2p5dg_jja_zonmean_lat-25n-75n_QRAindex.nc'.format(dir_in, exp, subexp, var, exp_dates[exp][subexp])
        ...
        for i,a in enumerate(files):
            ...
            a = a.replace('_QRAindex', '')
            if 'ENSEMBLE' in a: continue
            
            fname = a.split('/')[-1] 
            split = a.split('/')[-1].split('_')
                        
            if exp == 'cmip5': model_key = '{}_{}_null'.format(split[2], split[4])           # tas_Amon_ENSEMBLE-MEAN_historical_r1i1p1_185001-200512_2p5dg_jja_zonmean_lat-25n-75n.nc
            if exp == 'cmip6': model_key = '{}_{}_{}'  .format(split[2], split[4], split[5]) # tas_Amon_ENSEMBLE-MEAN_ssp585_r1i1p1f1_gn_201501-210012_2p5dg_jja_zonmean_lat-25n-75n.nc
                        
            ds  = xr.open_dataset(a, decode_times=False)
            dat = np.nanmean( np.squeeze(ds['tas'].data), axis=1)  - 273.15
            print(dat.shape, a)
...
data_ens_cmip5_rcp85  = np.nanmean(np.vstack(list(data_all_cmip5_rcp85.values())), axis=0) # data['cmip5']['rcp85']['ENSEMBLE-MEAN_r1i1p1_null']
...
data_ens_cmip6_hist   = np.nanmean(np.vstack(list(data_all_cmip6_hist.values())), axis=0) # data['cmip6']['historical']['ENSEMBLE-MEAN_r1i1p1f1_gn']
data_tim_cmip6_hist   = exp_years['cmip6']['historical']

data_all_cmip6_ssp585 = data['cmip6']['ssp585']
data_ens_cmip6_ssp585 = np.nanmean(np.vstack(list(data_all_cmip6_ssp585.values())), axis=0) # data['cmip6']['ssp585']['ENSEMBLE-MEAN_r1i1p1f1_gn']
...
data_ens_cmip6_ssp370 = np.nanmean(np.vstack(list(data_all_cmip6_ssp370.values())), axis=0) # data['cmip6']['ssp370']['ENSEMBLE-MEAN_r1i1p1f1_gn']
...
figname = '../figures/plot_serie_cmip5_historical_rcp85_tas_1850-2100.png'
...
figname = '../figures/plot_serie_cmip6_historical_ssp585_tas_1850-2100.png'
...
figname = '../figures/plot_serie_cmip6_historical_ssp370_tas_1850-2100.png'
...
figname = '../figures/plot_serie_cmip5_cmip6_historical_rcp_ssp_tas_1850-2100.png'
